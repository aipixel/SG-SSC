...
class TransformerPredictor(ME.MinkowskiNetwork):
    def __init__(self, config, in_channels_3d, in_channels_2d, num_classes, D=3):
        ...
        self.num_view = config.viewNum #5
        self.npb = config.num_point_batch #150
        self.hidden_dim = config.hidden_dim #128      num_tokens_2d = 88
        ...
        self.enc_share = TransformerEncoder(
            embed_dim=config.hidden_dim, #128
            depth=config.enc_layers, #6
            num_heads=config.nheads, #8
            drop_rate=config.drop_rate, #0.1
            attn_drop_rate=config.attn_drop_rate, #0.1
            drop_path=config.drop_path #0.1
        )

        self.dec_3d = TransformerDecoder(
            embed_dim=config.hidden_dim, #128
            depth=config.dec_layers, #6
    ...
    def forward(self, x_2d, x_3d):
        ...
        trans_input_3d = self.input_proj_3d(srcs_3d).transpose(1, 2).contiguous() # b n 128
        pe_3d = torch.stack(pos_batch, dim=0) # b n 128
        ...
        trans_input_2d = self.input_proj_2d(srcs_2d).transpose(1, 2).contiguous() # bv hw 128
        trans_input_2d = trans_input_2d.view(self.num_view, B, H*W, self.hidden_dim).transpose(0, 1).contiguous().view(B, N_2d, self.hidden_dim) # b vhw 128
